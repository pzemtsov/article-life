#ifndef _HASH_ADDITIVE_H
#define _HASH_ADDITIVE_H

#include "defs.h"
#include "cell.h"

template<size_t CAPACITY = 8192> class Hash_Additive: public Worker
{
private:
    HashCell**  table;
    HashCell full_list;
    HashCell * free_list;
    const static unsigned BASE = 946840871;
    const static HashType A = (HashType)(0x100000000L % 946840871);
    const static HashType B = 1;

public:
    Hash_Additive() : full_list(0, 0, 0), free_list(0)
    {
    ...
    {
        return (HashType)(key % BASE);
    }

    inline HashType add_hash(HashType hash, unsigned v) const
    {
        hash += v;
    ...
    inline HashType sub_hash(HashType hash, unsigned v) const
    {
        if (hash < v) hash += BASE;
        return hash - v;
    }
